...
- 🎯 **Códigos ICD que siguen al odontograma (especificidad basada en datos).** La profundidad de la caries proviene de la profundidad radiográfica cuando está registrada (E1/E2 → esmalte, D1–D3 → dentina) y recurre a la severidad ICDAS como respaldo (1–3 → esmalte, 4–6 → dentina), refinando el `K02` de la OMS a `K02.0` / `K02.1` y el `K02.9` de ICD-10-CM a `K02.51/.52` (fosas y fisuras) o `K02.61/.62` (superficie lisa) según superficie × profundidad. La periodontitis crónica toma su código ICD-10-CM del estadio y la extensión de 2017 (`K05.311`–`K05.329`). Una Condition por diente, con la afectación más profunda.
- 🔄 **Los datos periodontales van y vuelven por FHIR.** El importador lee de nuevo los paneles periodontales LOINC 74029-0 en cada diente — profundidad de sondaje, margen gingival (reconstruido a partir del CAL, de modo que los valores de pseudobolsa se conservan), BOP, furca, placa de O'Leary, los índices PI/GI y mPI/mBI de implante, y el ancho de encía queratinizada — además de las Observations de evidencia de tabaquismo y HbA1c. También se reconocen las Conditions codificadas solo en ICD-10-CM o SNOMED CT, así que un Bundle ajeno importa lo que puede.
- 🧬 **SNOMED CT para todo el catálogo de diagnósticos.** 48 de los 50 puestos llevan ahora un concepto SNOMED CT International verificado (activo, del módulo core, con la FSN correspondiente). Dos quedan deliberadamente sin asignar porque SNOMED International no tiene un concepto paraguas para ellos: anomalía del tamaño maxilar (K07.0) y alteraciones funcionales dentofaciales (K07.5). La superposición SNOMED sigue siendo opcional en Ajustes.
- 📦 **Un paquete de terminología FHIR cargable.** La carpeta `fhir/` del repositorio es un paquete NPM de FHIR (`react-advanced-odontogram.fhir`, FHIR 4.0.1) que contiene el CodeSystem del motor más los ValueSets generados — uno por cada grupo de valores de eje clínico, uno para los tipos de hallazgo y un conjunto con todos los códigos. Apunta un validador a él con `-ig ./fhir`.
...
- **Odontograma:** perfil de anatomía dental (`classic` por defecto / `measured`) — `measured` representa nueve plantillas dentales medidas según la literatura en una disposición de dos arcadas con ancho por diente; conmutable en tiempo de ejecución, sin efecto sobre el valor por defecto `classic`; su gráfico es un chunk aparte que solo se descarga al cambiar a él, de modo que el valor predeterminado `classic` no cuesta nada
...
npm run test:e2e       # Pruebas en navegador (Playwright; una vez: npx playwright install chromium)
```
...
Los datos periodontales ahora también van y vuelven por la importación FHIR, no solo por la carga JSON: `importFhirBundle()` lee de nuevo los paneles periodontales LOINC `74029-0` en el registro periodontal de cada diente — profundidad de sondaje, margen gingival (reconstruido a partir del CAL, de modo que los valores de pseudobolsa se conservan), BOP, furca, placa de O'Leary, los índices PI/GI y mPI/mBI de implante, y el ancho de encía queratinizada — además de las Observations de evidencia de tabaquismo y HbA1c a nivel de caso. La supuración es la única excepción: se queda solo en JSON, ya que no forma parte de la exportación FHIR.
...
- [@sofia-cluadette](https://github.com/sofia-cluadette): la API de selección `getSelectedTeeth()`
...
Las contribuciones son bienvenidas. Abre un pull request en GitHub y aparecerás aquí acreditado. Si el proyecto te resulta útil, por favor [dale una estrella en GitHub](https://github.com/ZoliQua/React-Advanced-Odontogram).
