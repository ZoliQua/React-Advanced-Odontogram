...
- 🔗 تصدير HL7 FHIR R4 (حزمة تجميعية (Bundle) من ملاحظات (Observations) لكل سن، مع ترميز أسنان وفق ISO 3950 للتسنين الدائم **و**اللبني (51-85، مع استيراد يعيد التخطيط دون فقدان بيانات)، ونظام ترميز محلي، بالإضافة إلى تراكب SNOMED CT اختياري (الإعدادات ← SNOMED CT))؛ كما يحمل مكوّن النخر (caries) الذي له شدة مسجَّلة ترميزًا إضافيًا لنظام التقييم — ICDAS على سطح أساسي (غير محشو)، وCARS على سطح ناكس (محشو)
...
- 🧬 ترميز تشخيصي قائم على معايير موحّدة (WHO ICD-10، مُفعَّل دائمًا): تشتق كل نتيجة مسجَّلة في المخطط تشخيصًا مرمَّزًا بـICD-10 — نخر الأسنان (K02)، تسوس سطح الجذر/الملاط والتسوس المتوقف (K02.2/.3)، التهاب اللب ونخر اللب (K04.0/.1)، التهاب دواعم ذروي وخراج حول الذروة وكيس جذري (K04.4–.9)، الحتّ/السحج/التآكل الحمضي/الكسر الإجهادي (K03.0–.8)، الجير السني (K03.6)، امتصاص السن (K03.3)، تغير لون السن (K00.3/K00.8/K03.7)، فقدان السن (K08.1)، بقايا جذر (K08.3)، وكسر السن (S02.5) — تُعرض في التلميح وملخص كامل الفم، وتُصدَّر كموارد FHIR من نوع Condition.
- 🩺 بطاقة التشخيصات لكل سن: عرض التشخيصات المشتقة وفق ICD-10 لسن معيّن وتنقيحها — إخفاء تشخيص مشتق خطأً أو إضافة تشخيص لا يمثّله المخطط. المجموعة الفعلية (المشتقة − المخفاة + المضافة) هي التي تقود تصدير FHIR.
- 🗺️ حالات الحالة/الإقليمية: تسجيل تشخيصات لكامل الفم غير مرتبطة بسن واحد — سوء الإطباق واضطراب المفصل الصدغي الفكي (K07)، الكيسات الفموية (K09)، أمراض الغدد اللعابية (K11)، التهاب الفم والمخاطية الفموية (K12/K13)، وشذوذات نمائية على مستوى القوس (K00) — كل منها قابل اختياريًا للتحديد الجانبي (يسار / يمين / ثنائي الجانب).
- 🌍 حزم الترميز الوطنية (الإعدادات ← نظام ترميز التشخيص): تراكب نظام ترميز وطني فوق أساس WHO ICD-10 — BNO-10 (المجر، عروض محلية؛ مع الاحتفاظ برمز WHO) أو US ICD-10-CM (رموز مُعاد تخطيطها، مثل نطاق K07 السني الوجهي ← M26). إضافة حزمة دولة أخرى هي إدخال صغير من نوع `CodingPack` — انظر `CODING_PACKS.md`.
- 🔬 تراكب SNOMED CT (الإعدادات ← SNOMED CT، اختياري، معطَّل افتراضيًا): يضيف ترميز SNOMED CT إلى جانب ترميز WHO وأي حزمة وطنية، ويرمّز نتائج ما حول الزرعة التي لا تملك رمز WHO ICD-10. معرّفات مفاهيم ICD-10-CM وSNOMED مرجعية/بأفضل جهد — يُرجى التحقق منها في القائمة الجدولية الرسمية لـICD-10-CM / متصفح SNOMED CT قبل الاستخدام السريري.
- 🔁 دورة FHIR Condition الكاملة: تُصدَّر التشخيصات كموارد FHIR من نوع `Condition` (مرتبطة بالسن، بالإضافة إلى حالات على مستوى المريض لها bodySite للجانبية) جنبًا إلى جنب مع الـObservations، ويعيد الاستيراد بناءها — حالات الحالة مباشرة، وتجاوزات الإضافة/الإخفاء لكل سن عبر مقارنة الـConditions المستوردة بالمخطط المعاد اشتقاقه.
- 🩺 **بطاقة التشخيصات، مُعاد تصميمها:** كل صف تشخيص لكل سن يعرض أولًا رمز ICD-10 (`K04.0 Pulpitis`) والصفوف مرتبة حسب الرمز. لكل صف مفتاح **استبعاد** (يحذف التشخيص من تصدير FHIR مع إبقائه على المخطط) وزر **حذف** (×) يحذف التشخيص *و* النتيجة المرتبطة به على السن. إضافة تشخيص من القائمة تكتب أيضًا النتيجة الأساسية في المخطط، فيظهر الرمز فورًا.
- 🗂️ **نافذة تشخيصات الحالة/الإقليمية المنبثقة:** انتقلت التشخيصات الخاصة بالفم كاملًا والتشخيصات الإقليمية (تشوهات الفكين، الكيسات، أمراض الغدد اللعابية والمخاطية…) من الشريط الجانبي لللثة إلى نافذة حوار خاصة بها، تُفتح من زر **التشخيصات** بجانب مفتاح التبديل بين المخطط السني وحالة اللثة؛ قائمتها مرتبة حسب الرمز مع تقديمه أولًا.
- 🇭🇺 **حزمة BNO-10:** أصبحت النصوص المعروضة بالمجرية هي العناوين الرسمية لـ BNO-10 الصادرة عن NEAK، وتستخدم الحزمة معرّف نظام ICD-10 القياسي (BNO-X مطابق لـICD-10 الصادر عن منظمة الصحة العالمية).
- ✅ **تصدير FHIR نظيف لمدققي HL7:** يحمل كل عنصر في الـBundle مُعرّف `id` ثابتًا و`fullUrl` مطلقًا (بلا عناصر نائبة `urn:uuid`)، ويضمّن الـBundle **CodeSystem** الخاص بالمحرك نفسه كي تُحل رموزه المحلية أثناء التحقق؛ وينشَر نفس الـCodeSystem في المستودع باسم `fhir/CodeSystem-odontogram.json` (مرّر `includeCodeSystem: false` في خيارات تصدير FHIR لحذفه).
- 🎯 **رموز ICD تتبع المخطط (دقة قائمة على البيانات).** يُستمد عمق التسوس من العمق الشعاعي عند تسجيله (E1/E2 ← مينا، D1–D3 ← عاج)، ويرجع إلى شدة ICDAS كبديل احتياطي (1–3 ← مينا، 4–6 ← عاج)، مما يُدقّق رمز WHO `K02` إلى `K02.0` / `K02.1`، ورمز ICD-10-CM `K02.9` إلى `K02.51/.52` (الحفر والشقوق) أو `K02.61/.62` (السطح الأملس) حسب السطح × العمق. ويستمد التهاب دواعم السن المزمن رمز ICD-10-CM الخاص به من المرحلة والمدى وفق تصنيف 2017 (`K05.311`–`K05.329`). عنصر Condition واحد لكل سن، يحمل أعمق إصابة.
- 🔄 **بيانات دواعم السن تدور الآن ذهابًا وإيابًا عبر FHIR.** يعيد الاستيراد قراءة لوحات دواعم السن LOINC 74029-0 إلى كل سن — عمق الجيب، والحافة اللثوية (المعاد بناؤها من CAL، بحيث تنجو قيم الجيب الكاذب)، وBOP، وتفرع الجذور، وبلاك أوليري، ومؤشرات PI/GI ومؤشرات الزرعة mPI/mBI، وعرض اللثة المتقرنة — إضافة إلى ملاحظات Observation الخاصة بحالة التدخين وHbA1c كدليل. كما تُعرَّف حالات Condition المرمَّزة فقط بـICD-10-CM أو SNOMED CT، بحيث تستورد أي حزمة خارجية ما تستطيعه.
- 🧬 **SNOMED CT لكامل كتالوج التشخيصات.** أصبح 48 من أصل 50 مدخلًا يحمل الآن مفهوم SNOMED CT International موثّقًا (نشِط، ضمن الوحدة الأساسية core، ومطابقًا للاسم الكامل FSN). ويبقى اثنان دون تعيين عمدًا لأن SNOMED International لا تملك مفهومًا جامعًا لهما: شذوذ حجم الفك (K07.0) والاضطرابات الوظيفية السنية الوجهية (K07.5). ويظل تراكب SNOMED اختياريًا في الإعدادات.
- 📦 **حزمة مصطلحات FHIR قابلة للتحميل.** مجلد `fhir/` في المستودع هو حزمة NPM لـFHIR (`react-advanced-odontogram.fhir`، الإصدار FHIR 4.0.1) تضم CodeSystem الخاص بالمحرك بالإضافة إلى ValueSets مُولَّدة — واحدة لكل مجموعة قيم لمحور سريري، وواحدة لأنواع النتائج، ومجموعة شاملة لكل الرموز. وجِّه إليها أداة تحقق عبر `-ig ./fhir`.
...
- **مخطط الأسنان:** ملف تشريح الأسنان (`classic` الافتراضي / `measured`) — يعرض `measured` تسعة قوالب أسنان مقاسة وفق الأدبيات في تخطيط بقوسين وعرض لكل سن؛ قابل للتبديل أثناء التشغيل، دون تأثير على الافتراضي `classic`؛ ورسوماته في حزمة منفصلة لا تُحمَّل إلا عند التبديل إليه، فلا يكلّف الوضع الافتراضي `classic` شيئًا
...
🅰️ **نسخة Angular:** يتوفر منفذ رسمي لـ Angular باسم [Angular Advanced Odontogram](https://github.com/ZoliQua/Angular-Advanced-Odontogram) (باسم `angular-advanced-odontogram` على npm) — يمكن تبادل صادرات JSON وFHIR R4 بين المكتبتين.
...
| `startIntroTour()` | إطلاق الجولة التعريفية التفاعلية من 18 خطوة |
...
- `dxOverrides` - تجاوزات ترميز التشخيص لكل سن (الإصدار 2.21): كائن مفتاحه مفتاح تشخيص ICD-10 ← `add` | `suppress`، يفرض تفعيل تشخيص مرمَّز رغم عدم وجود نتيجة مطابقة في المخطط، أو إيقافه رغم وجودها؛ يحدد المجموعة المرمَّزة الفعلية المصدَّرة كموارد `Condition` من FHIR
...
- `case` - كائن اختياري يحمل بيانات وصفية على مستوى الحالة (وليست لكل سن)، مشتركة بين مخططَي الحالة والخطة (يعكس مفتاح الحمولة العلوي `globals`). يُحذف عند الفراغ: يغيب كليًا عندما يكون كل حقل في قيمته الافتراضية، بحيث يبقى تصدير بلا بيانات حالة مطابقًا بايتًا بايت باستثناء رقم الإصدار. الحقول (يُحذف كل منها عند قيمته الافتراضية): `age`؛ `smokingStatus` (+ `cigarettesPerDay`)؛ `diabetesStatus` (+ `hba1c`)؛ `toothLossPerio`؛ `maxRblPercent`؛ تجاوزات السريري الأربعة لكل محور من تصنيف 2017 وهي `diagnosisOverride` / `stageOverride` / `gradeOverride` / `extentOverride`؛ و(من الإصدار 2.19) `patientName` / `examDate`؛ و(من الإصدار 2.20) `patientDob`؛ و(من الإصدار 2.22) `caseConditions` — تشخيصات الحالة/الإقليمية (سوء الإطباق واضطراب المفصل الصدغي الفكي K07، الكيسات الفموية K09، أمراض الغدد اللعابية K11، التهاب الفم والمخاطية الفموية K12/K13، وشذوذات نمائية على مستوى القوس K00)، كل منها مرتبط بجانبية (غير محدد / يسار / يمين / ثنائي الجانب). تغذّي هذه البيانات تصنيف مراحل/درجات دواعم السن ورأس تقرير PDF؛ وتُقرأ/تُكتب عبر `getCaseMeta()` ودوال `setCase*` (انظر واجهة برمجة التطبيقات العامة أعلاه). اسم المريض وتاريخ الميلاد وتاريخ الفحص بيانات هوية للمخطط فقط — وهي **ليست** جزءًا من تصدير FHIR.

### 🖨️ التصدير
تنتج `exportFhir()` حزمة (Bundle) نظيفة لمدققي HL7: يحمل كل عنصر فيها `id` ثابتًا و`fullUrl` مطلقًا (بلا عناصر نائبة `urn:uuid`)، وتضمّن الحزمة CodeSystem الخاص بالمحرك نفسه كي تُحل رموزه المحلية أثناء التحقق (وهو منشور أيضًا باسم `fhir/CodeSystem-odontogram.json` في المستودع؛ يمكن حذفه عبر `includeCodeSystem: false`).

بيانات دواعم السن تدور الآن أيضًا ذهابًا وإيابًا عبر استيراد FHIR، وليس فقط عبر حمولة JSON: تعيد `importFhirBundle()` قراءة لوحات دواعم السن LOINC `74029-0` إلى سجل دواعم السن الخاص بكل سن — عمق الجيب، والحافة اللثوية (المعاد بناؤها من CAL، بحيث تنجو قيم الجيب الكاذب)، وBOP، وتفرع الجذور، وبلاك أوليري، ومؤشرات PI/GI ومؤشرات الزرعة mPI/mBI، وعرض اللثة المتقرنة — إضافة إلى ملاحظات Observation الخاصة بحالة التدخين وHbA1c على مستوى الحالة. والاستثناء الوحيد هو التقيّح: يبقى في JSON فقط لأنه ليس جزءًا من تصدير FHIR.
