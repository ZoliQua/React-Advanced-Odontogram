...
🔗 **URL de test :** https://react-advanced-odontogram.vercel.app/
...
| `screenToothSpacing` | `"wide" \| "normal" \| "close"` | `"normal"` | Espacement entre les dents à l'écran. |
| `screenToothNumberSize` | `"small" \| "normal" \| "xlarge"` | `"normal"` | Taille du numéro de dent dans la grille. |
| `selectionColor` | `string` | `"#3b7bff"` | Couleur de l'anneau de sélection (`#rrggbb`). |
| `selectionBorderStyle` | `"solid" \| "dashed" \| "dotted"` | `"dashed"` | Style de bordure de l'anneau de sélection. |
| `toothInfo` | `boolean` | `true` | Afficher le panneau d'informations sur la dent. |
...
- 💾 Export/import de l'état en JSON (version 2.22 ; les imports acceptent encore les formats hérités 1.4 et 2.0 à 2.21 et migrent automatiquement, avec les états personnalisés des plugins et les notes par dent)
...
- 🔗 Export HL7 FHIR R4 (Bundle de collection d'Observations par dent, codage dentaire ISO 3950 pour la dentition permanente **et** les dents temporaires (51-85, aller-retour sans perte à l'import), système de codes local, plus une surcouche SNOMED CT optionnelle (Paramètres → SNOMED CT)) ; un composant de carie porteur d'une sévérité renseignée porte aussi le codage d'un système d'évaluation — ICDAS sur une surface primaire (non obturée), CARS sur une surface récidivante (obturée)
...
- 🧬 Codage diagnostique basé sur des standards (WHO ICD-10, toujours actif) : chaque observation renseignée dérive un diagnostic codé en ICD-10 — carie (K02), carie radiculaire/cémentaire et carie arrêtée (K02.2/.3), pulpite et nécrose pulpaire (K04.0/.1), parodontite apicale, abcès périapical et kyste radiculo-dentaire (K04.4–.9), attrition/abrasion/érosion/abfraction (K03.0–.8), tartre (K03.6), résorption (K03.3), coloration (K00.3/K00.8/K03.7), perte dentaire (K08.1), racine résiduelle (K08.3) et fracture dentaire (S02.5) — affiché dans l'infobulle et le résumé bucco-dentaire, et exporté sous forme de FHIR Conditions.
- 🩺 Carte des Diagnostics par dent : consulter les diagnostics ICD-10 dérivés d'une dent et les organiser — supprimer un diagnostic mal dérivé ou en ajouter un que le schéma ne représente pas. L'ensemble effectif (dérivés − supprimés + ajoutés) pilote l'export FHIR.
- 🗺️ Conditions de cas / régionales : renseigner des diagnostics bucco-dentaires non liés à une dent précise — malocclusion et troubles de l'ATM (K07), kystes de la région buccale (K09), maladies des glandes salivaires (K11), stomatite et muqueuse buccale (K12/K13), et anomalies du développement à l'échelle de l'arcade (K00) — chacune éventuellement latéralisée (gauche / droite / bilatérale).
- 🌍 Packs de codage nationaux (Paramètres → Codage du diagnostic) : superpose un système de codes national à la base WHO ICD-10 — BNO-10 (hongrois, libellés localisés ; conserve le code WHO) ou US ICD-10-CM (codes remappés, p. ex. la plage dentofaciale K07 → M26). Ajouter le pack d'un autre pays est une petite entrée `CodingPack` — voir `CODING_PACKS.md`.
- 🔬 Surcouche SNOMED CT (Paramètres → SNOMED CT, optionnelle, désactivée par défaut) : ajoute un codage SNOMED CT en plus du codage WHO et de tout pack national, et code les observations péri-implantaires qui n'ont pas de code WHO ICD-10. Les identifiants de concept ICD-10-CM et SNOMED sont indicatifs/de bonne foi — à vérifier par rapport à la liste tabulaire officielle ICD-10-CM / au navigateur SNOMED CT avant toute utilisation clinique.
- 🔁 Aller-retour FHIR Condition : les diagnostics s'exportent sous forme de ressources FHIR `Condition` (liées à une dent, plus des conditions de cas au niveau du patient avec un `bodySite` de latéralité) aux côtés des Observations, et l'import les reconstruit — les conditions de cas directement, et les surcharges d'ajout/suppression par dent en comparant les Conditions importées au schéma re-dérivé.
- 🩺 **Carte des diagnostics, revue :** chaque ligne de diagnostic par dent affiche d'abord le code ICD-10 (`K04.0 Pulpitis`) et les lignes sont triées par code. Chaque ligne a un interrupteur **exclure** (retire le diagnostic de l'export FHIR mais le garde sur le schéma) et un bouton **supprimer** (×) qui retire le diagnostic *et* le constat correspondant sur la dent. Ajouter un diagnostic depuis le sélecteur écrit aussi le constat sous-jacent, si bien que le glyphe apparaît immédiatement.
- 🗂️ **Fenêtre des diagnostics de cas / régionaux :** les diagnostics de bouche entière et régionaux (anomalies maxillaires, kystes, affections salivaires et de la muqueuse…) ont quitté la barre latérale parodontale pour leur propre boîte de dialogue, ouverte depuis le bouton **Diagnostics** à côté du bascule Odontogramme / État parodontal ; son sélecteur est trié par code, code en tête.
- 🇭🇺 **Paquet BNO-10 :** les libellés hongrois affichés sont désormais les intitulés officiels NEAK BNO-10, et le paquet utilise l'URI de système ICD-10 standard (BNO-X est identique à l'ICD-10 de l'OMS).
- ✅ **Export FHIR propre pour les validateurs HL7 :** chaque entrée du Bundle porte un `id` déterministe et une `fullUrl` absolue (pas d'espace réservé `urn:uuid`), et le Bundle intègre le **CodeSystem** propre au moteur pour que ses codes locaux se résolvent lors de la validation ; le même CodeSystem est publié dans le dépôt sous `fhir/CodeSystem-odontogram.json` (passez `includeCodeSystem: false` dans les options d'export FHIR pour l'omettre).
- 🎯 **Des codes ICD qui suivent le schéma (spécificité pilotée par les données).** La profondeur de la carie provient de la profondeur radiographique lorsqu'elle est chartée (E1/E2 → émail, D1–D3 → dentine), avec un repli sur la sévérité ICDAS (1–3 → émail, 4–6 → dentine), affinant le `K02` de l'OMS en `K02.0` / `K02.1` et le `K02.9` ICD-10-CM en `K02.51/.52` (puits et fissures) ou `K02.61/.62` (surface lisse) selon surface × profondeur. La parodontite chronique tire son code ICD-10-CM du stade et de l'étendue 2017 (`K05.311`–`K05.329`). Une Condition par dent, portant l'atteinte la plus profonde.
- 🔄 **Les données parodontales font l'aller-retour par FHIR.** L'import relit les panels parodontaux LOINC 74029-0 dans chaque dent — profondeur de sondage, marge gingivale (reconstruite à partir du CAL, si bien que les valeurs de pseudo-poche survivent), BOP, furcation, plaque O'Leary, les indices PI/GI et mPI/mBI d'implant, et la largeur de gencive kératinisée — plus les Observations de preuve tabagisme et HbA1c. Les Conditions codées uniquement en ICD-10-CM ou en SNOMED CT sont également reconnues, si bien qu'un Bundle étranger importe ce qu'il peut.
- 🧬 **SNOMED CT pour tout le catalogue de diagnostics.** 48 des 50 emplacements portent désormais un concept SNOMED CT International vérifié (actif, module core, FSN correspondante). Deux restent délibérément non renseignés car SNOMED International n'a pas de concept ombrelle pour eux : anomalie de taille maxillaire (K07.0) et anomalies fonctionnelles dento-faciales (K07.5). La surcouche SNOMED reste optionnelle dans les Paramètres.
- 📦 **Un pack de terminologie FHIR chargeable.** Le dossier `fhir/` du dépôt est un pack NPM FHIR (`react-advanced-odontogram.fhir`, FHIR 4.0.1) qui contient le CodeSystem du moteur ainsi que des ValueSets générés — un par groupe de valeurs d'axe clinique, un pour les types d'observation, et un ensemble regroupant tous les codes. Pointez un validateur dessus avec `-ig ./fhir`.
...
- **Odontogramme :** profil d'anatomie dentaire (`classic` par défaut / `measured`) — `measured` affiche neuf modèles dentaires mesurés d'après la littérature dans une disposition à deux arcades avec largeur par dent ; commutable à l'exécution, sans effet sur la valeur par défaut `classic` ; son graphisme forme un chunk distinct, téléchargé uniquement lors du basculement, si bien que le défaut `classic` ne coûte rien
...
🅰️ **Version Angular :** un portage officiel pour Angular, [Angular Advanced Odontogram](https://github.com/ZoliQua/Angular-Advanced-Odontogram) (`angular-advanced-odontogram` sur npm), est disponible — les exports JSON et FHIR R4 sont interopérables entre les deux librairies.
...
| `getSelectedTeeth()` | Dents actuellement sélectionnées (numéros FDI), dans l'ordre de sélection |
| `getNumberingSystem()` | Le système de numérotation dentaire actif |
| `getScreenToothSpacing()` / `setScreenToothSpacing(v)` | Lit/définit l'espacement entre les dents à l'écran |
| `getScreenToothNumberSize()` / `setScreenToothNumberSize(v)` | Lit/définit la taille du numéro de dent |
| `getSelectionColor()` / `setSelectionColor(hex)` | Lit/définit la couleur de l'anneau de sélection (`#rrggbb`) |
| `getSelectionBorderStyle()` / `setSelectionBorderStyle(v)` | Lit/définit le style de bordure de l'anneau de sélection |
| `getToothInfoVisible()` / `setToothInfoVisible(on)` | Lit/définit si le panneau d'informations sur la dent est affiché |
...
| `startIntroTour()` | Lance la visite guidée interactive en 18 étapes |
...
L'export crée un fichier JSON (version `2.22` ; les imports acceptent aussi les formats hérités `1.4` et `2.0` à `2.21` et migrent automatiquement) contenant :
...
- `dxOverrides` - substitutions de codage diagnostique par dent (version 2.21) : un objet indexé par clé de diagnostic ICD-10 → `add` | `suppress`, forçant l'activation d'un diagnostic codé malgré l'absence d'observation correspondante, ou sa désactivation malgré sa présence ; détermine l'ensemble codé effectif exporté sous forme de `Condition`s FHIR
...
**Champ `case` de premier niveau (version 2.17+, étendu en 2.18, 2.19, 2.20 et 2.22) :**
- `case` - objet optionnel contenant des métadonnées au niveau du cas (non par dent), partagées par les schémas d'état et de plan (reflète la clé `globals` de premier niveau). Omis quand vide : entièrement absent lorsque tous les champs sont à leur valeur par défaut, de sorte qu'un export sans cas reste identique octet pour octet à l'exception du numéro de version. Champs (chacun omis lorsqu'à sa valeur par défaut) : `age` ; `smokingStatus` (+ `cigarettesPerDay`) ; `diabetesStatus` (+ `hba1c`) ; `toothLossPerio` ; `maxRblPercent` ; les quatre surcharges par axe du clinicien de la classification 2017 `diagnosisOverride` / `stageOverride` / `gradeOverride` / `extentOverride` ; (version 2.19) `patientName` / `examDate` ; et (version 2.20) `patientDob` ; et (version 2.22) `caseConditions` — diagnostics du cas/régionaux (malocclusion et ATM K07, kystes buccaux K09, maladies des glandes salivaires K11, stomatite et muqueuse buccale K12/K13, anomalies du développement au niveau de l'arcade K00), chacun associé à une latéralité (non précisé / gauche / droit / bilatéral). Il alimente la classification de stadification/gradation parodontale et l'en-tête du rapport PDF ; lu/écrit via `getCaseMeta()` et les setters `setCase*` (voir l'API publique ci-dessus). Le nom du patient, la date de naissance et la date d'examen ne sont que des métadonnées d'identité du schéma — ils ne font **pas** partie de l'export FHIR.

### 🖨️ Export
`exportFhir()` produit un Bundle propre pour les validateurs HL7 : chaque entrée porte un `id` déterministe et une `fullUrl` absolue (pas d'espace réservé `urn:uuid`), et le Bundle intègre le CodeSystem propre au moteur pour que ses codes locaux se résolvent lors de la validation (également publié sous `fhir/CodeSystem-odontogram.json` dans le dépôt ; omissible via `includeCodeSystem: false`).

Les données parodontales font désormais aussi l'aller-retour par l'import FHIR, pas seulement par la charge utile JSON : `importFhirBundle()` relit les panels parodontaux LOINC `74029-0` dans le dossier parodontal de chaque dent — profondeur de sondage, marge gingivale (reconstruite à partir du CAL, si bien que les valeurs de pseudo-poche survivent), BOP, furcation, plaque O'Leary, les indices PI/GI et mPI/mBI d'implant, et la largeur de gencive kératinisée — plus les Observations de preuve tabagisme et HbA1c au niveau du cas. La suppuration est la seule exception : elle reste JSON uniquement, car elle ne fait pas partie de l'export FHIR.
...
- [@odontodev](https://github.com/odontodev): hydratation de l'état et API de cycle de vie, réglages des obturations en tant que props contrôlées, setters idempotents et cartes repliables; les réglages d'affichage du schéma et `getNumberingSystem()` exposés à l'application hôte, ainsi que les notifications `onStateChange` manquantes pour les réglages de session et les notes par dent
...
- [@saegerdirk-star](https://github.com/saegerdirk-star): anatomie dentaire mesurée et le générateur de dents, ainsi que la proposition d'interface composable; trois corrections reprises de leur fork (identité du patient dans le PDF, orientation des dents dans le parodontogramme, vitesse de sélection)
- [@sofia-cluadette](https://github.com/sofia-cluadette): l'API de sélection `getSelectedTeeth()`
...
Les contributions sont les bienvenues. Ouvrez une pull request sur GitHub et vous serez crédité ici. Si le projet vous est utile, merci de [lui donner une étoile sur GitHub](https://github.com/ZoliQua/React-Advanced-Odontogram).
