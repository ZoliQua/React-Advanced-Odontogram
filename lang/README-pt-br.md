...
| `screenToothSpacing` | `"wide" \| "normal" \| "close"` | `"normal"` | Espaçamento entre dentes na tela. |
| `screenToothNumberSize` | `"small" \| "normal" \| "xlarge"` | `"normal"` | Tamanho do número do dente na grade. |
| `selectionColor` | `string` | `"#3b7bff"` | Cor do anel de seleção (`#rrggbb`). |
| `selectionBorderStyle` | `"solid" \| "dashed" \| "dotted"` | `"dashed"` | Estilo da borda do anel de seleção. |
| `toothInfo` | `boolean` | `true` | Exibir o painel de informações do dente. |
...
- 🧬 Codificação de diagnóstico baseada em padrões (WHO ICD-10, sempre ativa): cada achado registrado no gráfico deriva um diagnóstico codificado em ICD-10 — cárie dentária (K02), cárie radicular/cementária e cárie paralisada (K02.2/.3), pulpite e necrose pulpar (K04.0/.1), periodontite apical, abscesso periapical e cisto radicular (K04.4–.9), atrição/abrasão/erosão/abfração (K03.0–.8), cálculo dentário (K03.6), reabsorção dentária (K03.3), descoloração dentária (K00.3/K00.8/K03.7), perda dentária (K08.1), raiz residual (K08.3) e fratura dentária (S02.5) — exibido no tooltip e no resumo de boca completa, e exportado como FHIR Conditions.
- 🩺 Cartão de Diagnósticos por dente: veja os diagnósticos ICD-10 derivados de um dente e cure-os — suprima um derivado incorretamente ou adicione um que o gráfico não representa. O conjunto efetivo (derivado − suprimido + adicionado) direciona a exportação FHIR.
- 🗺️ Condições de caso/regionais: registre diagnósticos de boca completa não vinculados a um único dente — má oclusão e transtornos da articulação temporomandibular (K07), cistos orais (K09), doenças da glândula salivar (K11), estomatite e mucosa oral (K12/K13), e anomalias de desenvolvimento em nível de arcada (K00) — cada uma opcionalmente lateralizada (esquerda / direita / bilateral).
- 🌍 Pacotes de codificação nacionais (Configurações → Sistema de codificação de diagnóstico): sobrepõe um sistema de código nacional à base WHO ICD-10 — BNO-10 (húngaro, exibições localizadas; mantém o código WHO) ou US ICD-10-CM (códigos remapeados, ex.: a faixa dentofacial K07 → M26). Adicionar o pacote de outro país é uma pequena entrada `CodingPack` — veja `CODING_PACKS.md`.
- 🔬 Sobreposição SNOMED CT (Configurações → SNOMED CT, opcional, desativada por padrão): adiciona uma codificação SNOMED CT junto à codificação WHO e de qualquer pacote nacional, e codifica os achados peri-implantares que não têm código WHO ICD-10. Os IDs de conceito ICD-10-CM e SNOMED são de referência/melhor esforço — verifique na lista tabular oficial do ICD-10-CM / no navegador do SNOMED CT antes do uso clínico.
- 🔁 Round-trip de Condition FHIR: os diagnósticos são exportados como recursos FHIR `Condition` (vinculados ao dente, além de condições de caso em nível de paciente com um bodySite de lateralidade) junto com as Observations, e a importação os reconstrói — as condições de caso diretamente, e as substituições de adição/supressão por dente comparando as Conditions importadas com o gráfico rederivado.
- 🩺 **Cartão de diagnósticos, revisado:** cada linha de diagnóstico por dente mostra primeiro o código ICD-10 (`K04.0 Pulpitis`) e as linhas são ordenadas por código. Cada linha tem um alternador de **excluir** (remove o diagnóstico da exportação FHIR, mas o mantém no gráfico) e um botão de **apagar** (×) que remove o diagnóstico *e* o achado correspondente no dente. Adicionar um diagnóstico pelo seletor também grava o achado subjacente no gráfico, então o símbolo aparece imediatamente.
- 🗂️ **Janela de diagnósticos de caso/regionais:** os diagnósticos de boca completa e regionais (anomalias maxilares, cistos, condições salivares e da mucosa…) saíram da barra lateral periodontal para um diálogo próprio, aberto pelo botão **Diagnósticos** ao lado do alternador Odontograma / Status periodontal; seu seletor é ordenado por código, com o código em primeiro lugar.
- 🇭🇺 **Pacote BNO-10:** os textos exibidos em húngaro agora são os títulos oficiais BNO-10 do NEAK, e o pacote usa o URI padrão do sistema ICD-10 (BNO-X é idêntico ao ICD-10 da OMS).
- ✅ **Exportação FHIR limpa para validadores HL7:** cada entrada do Bundle carrega um `id` determinístico e um `fullUrl` absoluto (sem placeholders `urn:uuid`), e o Bundle incorpora o próprio **CodeSystem** do motor para que seus códigos locais sejam resolvidos durante a validação; o mesmo CodeSystem é publicado no repositório como `fhir/CodeSystem-odontogram.json` (passe `includeCodeSystem: false` nas opções de exportação FHIR para omiti-lo).
- 🎯 **Códigos ICD que acompanham o odontograma (especificidade orientada por dados).** A profundidade da cárie vem da profundidade radiográfica quando registrada (E1/E2 → esmalte, D1–D3 → dentina) e recorre à gravidade ICDAS como alternativa (1–3 → esmalte, 4–6 → dentina), refinando o `K02` da OMS para `K02.0` / `K02.1` e o `K02.9` do ICD-10-CM para `K02.51/.52` (fóssulas e fissuras) ou `K02.61/.62` (superfície lisa) por superfície × profundidade. A periodontite crônica recebe seu código ICD-10-CM a partir do estágio e da extensão de 2017 (`K05.311`–`K05.329`). Uma Condition por dente, carregando o envolvimento mais profundo.
- 🔄 **Os dados periodontais agora fazem o caminho de volta pelo FHIR.** A importação lê de volta os painéis periodontais LOINC 74029-0 em cada dente — profundidade de sondagem, margem gengival (reconstruída a partir do CAL, então valores de pseudobolsa sobrevivem), BOP, furca, placa de O'Leary, os índices PI/GI e mPI/mBI de implante, e a largura de gengiva queratinizada — além das Observations de evidência de tabagismo e HbA1c. Conditions codificadas apenas em ICD-10-CM ou SNOMED CT também são reconhecidas, então um bundle externo importa o que puder.
- 🧬 **SNOMED CT para todo o catálogo de diagnósticos.** 48 dos 50 slots agora carregam um conceito SNOMED CT International verificado (ativo, no módulo core, com FSN correspondente). Dois permanecem deliberadamente vazios porque a SNOMED International não tem um conceito guarda-chuva para eles: anomalia de tamanho maxilar (K07.0) e anormalidades funcionais dentofaciais (K07.5). A sobreposição SNOMED continua opcional em Configurações.
- 📦 **Um pacote de terminologia FHIR carregável.** A pasta `fhir/` do repositório é um pacote NPM de FHIR (`react-advanced-odontogram.fhir`, FHIR 4.0.1) contendo o CodeSystem do motor mais ValueSets gerados — um por grupo de valores de eixo clínico, um para os tipos de achado e um conjunto com todos os códigos. Aponte um validador para ele com `-ig ./fhir`.
...
- **Odontogram:** perfil de anatomia dental (`classic` padrão / `measured`) — `measured` renderiza nove modelos de dentes medidos pela literatura em um layout de dois arcos com largura por dente; alternável em tempo de execução, sem efeito sobre o padrão `classic`; sua arte é um chunk separado, baixado apenas ao alternar para ele, de modo que o padrão `classic` não custa nada
...
🅰️ **Versão para Angular:** há um port oficial para Angular, o [Angular Advanced Odontogram](https://github.com/ZoliQua/Angular-Advanced-Odontogram) (`angular-advanced-odontogram` no npm) — as exportações JSON e FHIR R4 são compatíveis entre as duas bibliotecas.
...
| `getSelectedTeeth()` | Dentes selecionados no momento (números FDI), na ordem de seleção |
| `getNumberingSystem()` | O sistema de numeração dentária ativo |
| `getScreenToothSpacing()` / `setScreenToothSpacing(v)` | Ler/definir o espaçamento entre dentes na tela |
| `getScreenToothNumberSize()` / `setScreenToothNumberSize(v)` | Ler/definir o tamanho do número do dente |
| `getSelectionColor()` / `setSelectionColor(hex)` | Ler/definir a cor do anel de seleção (`#rrggbb`) |
| `getSelectionBorderStyle()` / `setSelectionBorderStyle(v)` | Ler/definir o estilo da borda do anel de seleção |
| `getToothInfoVisible()` / `setToothInfoVisible(on)` | Ler/definir se o painel de informações do dente é exibido |
...
| `startIntroTour()` | Inicia o tour interativo de introdução em 18 passos |
...
- `dxOverrides` - sobrescritas de codificação diagnóstica por dente (versão 2.21): um objeto indexado pela chave de diagnóstico ICD-10 → `add` | `suppress`, forçando um diagnóstico codificado mesmo sem um achado correspondente no gráfico, ou suprimindo-o mesmo havendo um; molda o conjunto codificado efetivo exportado como `Condition`s do FHIR
...
- `case` - objeto opcional contendo metadados em nível de caso (não por dente), compartilhado por ambos os gráficos de status e plano (espelha a chave `globals` de nível superior). Omite-quando-vazio: ausente por completo quando cada campo está em seu padrão, de modo que uma exportação sem caso permanece idêntica byte a byte exceto pelo número de versão. Campos (cada um omitido quando em seu padrão): `age`; `smokingStatus` (+ `cigarettesPerDay`); `diabetesStatus` (+ `hba1c`); `toothLossPerio`; `maxRblPercent`; as quatro sobrescritas clínicas por eixo da classificação 2017 `diagnosisOverride` / `stageOverride` / `gradeOverride` / `extentOverride`; (versão 2.19) `patientName` / `examDate`; e (versão 2.20) `patientDob`; e (versão 2.22) `caseConditions` — diagnósticos de caso/regionais (má oclusão e transtornos da articulação temporomandibular K07, cistos orais K09, doenças da glândula salivar K11, estomatite e mucosa oral K12/K13, anomalias de desenvolvimento em nível de arcada K00), cada um mapeado a uma lateralidade (não especificada / esquerda / direita / bilateral). Ele alimenta a classificação de estadiamento/graduação periodontal e o cabeçalho do relatório PDF; lido/escrito via `getCaseMeta()` e os setters `setCase*` (veja API pública acima). Nome do paciente, data de nascimento e data do exame são apenas metadados de identidade do gráfico — eles **não** fazem parte da exportação FHIR.

### 🖨️ Exportação
`exportFhir()` gera um Bundle limpo para validadores HL7: cada entrada carrega um `id` determinístico e um `fullUrl` absoluto (sem placeholders `urn:uuid`), e o Bundle incorpora o próprio CodeSystem do motor para que seus códigos locais sejam resolvidos durante a validação (também publicado como `fhir/CodeSystem-odontogram.json` no repositório; pode ser omitido com `includeCodeSystem: false`).

Os dados periodontais agora também fazem o caminho de volta pela importação FHIR, não apenas pelo payload JSON: `importFhirBundle()` lê de volta os painéis periodontais LOINC `74029-0` no registro periodontal de cada dente — profundidade de sondagem, margem gengival (reconstruída a partir do CAL, então valores de pseudobolsa sobrevivem), BOP, furca, placa de O'Leary, os índices PI/GI e mPI/mBI de implante, e a largura de gengiva queratinizada — além das Observations de evidência de tabagismo e HbA1c em nível de caso. A supuração é a única exceção: permanece apenas em JSON, já que não faz parte da exportação FHIR.
...
- [@odontodev](https://github.com/odontodev): hidratação de estado e API de ciclo de vida, configurações de restaurações como props controladas, setters idempotentes e cartões recolhíveis; as configurações de exibição do odontograma e `getNumberingSystem()` expostas ao aplicativo hospedeiro, além das notificações `onStateChange` que faltavam para as configurações de sessão e as notas por dente
...
- [@saegerdirk-star](https://github.com/saegerdirk-star): anatomia dental medida e o gerador de dentes, além da proposta de interface componível; três correções adotadas do seu fork (identidade do paciente no PDF, orientação dos dentes no periograma, velocidade de seleção)
- [@sofia-cluadette](https://github.com/sofia-cluadette): a API de seleção `getSelectedTeeth()`
...
As contribuições são bem-vindas. Abra um pull request no GitHub e você será creditado aqui. Se o projeto for útil para você, por favor [dê uma estrela no GitHub](https://github.com/ZoliQua/React-Advanced-Odontogram).
